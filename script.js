...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Function to close a modal when clicking outside of it
function setupModalCloseOnOutsideClick(modal) {
  modal.addEventListener('click', function (event) {
      if (event.target === modal) { // Check if the click is on the modal background
          ...
          clearInterval(currentInterval); // Stop any active intervals if applicable
      }
  });
  ...
  // Apply the above function to each modal
setupModalCloseOnOutsideClick(passwordModal);
setupModalCloseOnOutsideClick(clockInModal);
setupModalCloseOnOutsideClick(editEmployeeModal);
setupModalCloseOnOutsideClick(addEmployeeModal);
setupModalCloseOnOutsideClick(editTimesheetModal);
setupModalCloseOnOutsideClick(detailedViewModal);
